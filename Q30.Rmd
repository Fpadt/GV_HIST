...
dtUKa <- dtMTSLS[dtUK[, .(BILL_NUM, MATERIAL, DISTR_CHAN, SALESORG)], 
                 on= c("MATERIAL", "DISTR_CHAN", "SALESORG")][, .N, by = .(BILL_NUM, AAG)]

dtUKa <- dtAAG[dtUKa, on="AAG"]
setcolorder(dtUKa, c("BILL_NUM", "AAG", "BIT", "N"))
setkey(dtUKa, "BILL_NUM", "AAG")

dtUKa <- dtUKa[, G1AAGCMB:= sum(BIT), by = .(BILL_NUM)][, .N, by =.(BILL_NUM, G1AAGCMB)]
...
### Prepare for Export ####
dtUK <- fAlignDS(pDT =  dtUK, pDS = "G1_DS_PA_HIST_FLAT_FILE")
